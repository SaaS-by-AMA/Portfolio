...
export function MenuItem({ children, onClick, disabled = false, icon, isActive = false }: MenuItemProps) {
    return (
        ...
        ${disabled ? "text-[var(--text-dim)] cursor-not-allowed" : "text-[var(--text-muted)] hover:text-[var(--primary)]"}
    ...
    return (
        <div className="relative w-[64px]" data-expanded={isExpanded}>
            ...
            <div className="relative">
                ...
                <div
                    className="relative w-16 h-16 bg-[var(--surface)] border border-[var(--surface-highlight)] cursor-pointer rounded-full group will-change-transform z-50"
                ...
                {childrenArray.slice(1).map((child, index) => (
                    <div
                        key={index}
                        className="absolute top-0 left-0 w-16 h-16 bg-[var(--surface)] border border-[var(--surface-highlight)] will-change-transform"
                        style={{
